...
The improved version of OSC must disable the built-in OSC of the original mpv and is not compatible with other OSC scripts.
Integrated thumbnail script function (thumbfast.lua), you need to download and use it yourself: https://github.com/po5/thumbfast
An example is written in input.conf:
SHIFT+DEL script-binding osc_lazy/visibility # Switch the visibility of osc_lazy
--]] 

function lock_osc(name, value)
    if value == true then
    mp.set_property("osc", "no")
    mp.msg.info("The script has automatically executed osc=no for compatibility")
    end
    end
    ...
    layout = "bottombar", -- The original version can be "bottombar" "topbar" "box" "slimbox"; add "bottombox" to osc_lazy
    ...
    seekbarkeyframes = true, -- use keyframes when dragging the seekbar -- now not controlled by global hr-seek
    title = "${media-title}", -- string compatible with property-expansion
    -- to be shown as OSC title
    tooltipborder = 1, -- border of tooltip in bottom/topbar
    timetotal = true, -- display total time instead of remaining time? -- The original version is false
    timems = false, -- display timecodes with milliseconds?
    visibility = "auto", -- only used at init to set visibility_mode(...)
    boxmaxchars = 150, -- title crop threshold for box layout -- original version is 80
    ...
    chapter_fmt = "Chapter: %s", -- chapter print format for seekbar-hover. "no" to disable 
     ...
     wctitle = "${media-title}", -- Borderless top title
    sub_title = " ", -- the right subtitle of the bottombox layout
    sub_title2 = "Contrast[${contrast}] Brightness[${brightness}] Gamma[${gamma}] Saturation[${saturation}] Hue[${hue}]", -- Temporary right sub of bottombox layout title
    seekbar_scrollseek = "fast", -- The scroll wheel jump mode of the progress bar "fast" "second" "frame". Not controlled by global hr-seek
    showonpause = false, -- Permanent OSC when paused
    showonstart = false, -- Display OSC at the beginning of playback or when playing the next file
    showonseek = false, -- show OSC when jumping
    font = "sans", -- OSC's global font display
    font_mono = "sans",
    font_bold = 500,
    } 
    ...
    lastvisibility = user_opts.visibility, -- if showonpause, save the last visibility when paused
    } 
    ...
    function get_hitbox_coords(x, y, an, w, h) 
     ...
     local alignments = {
    [1] = function () return x, yh, x+w, y end,
    [2] = function () return x-(w/2), yh, x+(w/2), y end,
    [3] = function () return xw, yh, x, y end, 
    
     [4] = function () return x, y-(h/2), x+w, y+(h/2) end,
    [5] = function () return x-(w/2), y-(h/2), x+(w/2), y+(h/2) end,
    [6] = function () return xw, y-(h/2), x, y+(h/2) end, 
    
     [7] = function () return x, y, x+w, y+h end,
    [8] = function () return x-(w/2), y, x+(w/2), y+h end,
    [9] = function () return xw, y, x, y+h end,
    } 
    ...
    -- 
     ...
     local nicetypes = {video = "video", audio = "audio", sub = "subtitles"} 
    ...
    local msg = "available" .. nicetypes[type] .. "rail:"
    ...
    new_track_mpv = tracks_osc[type][new_track_osc].id
     ...
     if (new_track_osc == 0) then
    show_message(nicetypes[type] .. "Track: < disabled >")
    else
    ...
    element.eventresponder = nil
     ...
     -- Gray out related elements when disabled
    if (element.off) then
    element.layout.alpha[1] = 136
    end
    end
    ...
    state.forced_title = string.format(user_opts.chapter_fmt, ch.title)
     ...
     -- Temporary display of the subtitle on the right side of the bottombox 
    ...
    return 'No chapters.'
     ...
     local message = string.format('Chapter list[%d/%d]\n', pos, count)
    for i, v in ipairs(limlist) do
    ...
    p = 15, -- padding
     ...
     --The overall position of the bottombox
    ...
    function () show_message(mp.get_property_osd("filename")) end 
     ...
     -- sub_title -- The right subtitle of the bottombox
    ...
    function () mp.commandv("cycle", "pause") end 
     ...
     --skipback -- progress (roll forward)
    ...
    update_tracklist() 
     ...
     --cy_audio --Global audio track button enhancement
    ...
    function () set_track("audio", 1) end 
     ...
     --cy_sub --Global subtitle button enhancement
    ...
    function () set_track("sub", 1) end 
     ...
     --tog_fs --Switch full screen/windowed
    ...
    function () mp.commandv("cycle", "fullscreen") end 
     ...
     --seekbar --Global progress bar enhancement
    ...
    -- seeks identical
    thumbfast.pause = false --Pause rendering of thumbnails
    mp.commandv("script-message-to", "thumbfast", "clear")
    local seekto = get_slider_value(element)
    if (element.state.lastseek == nil) or
    (not (element.state.lastseek == seekto)) then
    local flags = "absolute-percent+keyframes" -- prevent the impact of --hr-seek
    if not user_opts.seekbarkeyframes then
    flags = "absolute-percent+exact" -- prevent the impact of --hr-seek
    end
    ...
    function (element) thumbfast.pause = true end --Resume rendering thumbnails
    ...
    elseif user_opts.seekbar_scrollseek == "frame" then mp.commandv('frame-step')
     ...
     -- tc_left (current pos) -- current time
    ...
    request_init()
     ...
     -- tc_right (total/remaining time) -- total/remaining time
    ...
    local sec = math.floor(dmx_cache % 60) -- don't round eg 59.9 to 60
    local cache_speed = string.format("%.2f",(mp.get_property_native("cache-speed") / 1048576))
    return "buffer:" .. (min > 0 and
    string.format("%sm%02.0fs", min, sec) or
    string.format("%3.0fs", sec)) .. (cache_speed ~= nil and (" speed: " .. cache_speed .. " MB/s") or "")
    end 
    ...
    function () mp.commandv("osd-auto", "add", "volume", -1) end 
     ...
     --The bottombox statistics button is linked to the built-in stats.lua
    ...
    function show_osc()
    -- show when disabled can happen (eg mouse_move) due to async/delayed unbinding
    ...
    ass:an(8)
    ass:append("Drag into files/directories/websites to play")
    end
