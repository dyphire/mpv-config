...
function lock_osc(name, value)
    if value == true then
        ...
        mp.msg.info("The script has automatically implemented osc=no for compatibility")
    end
end
...
local user_opts = {
    ...
    layout = "bottombar",               -- the original options are "bottombar" "topbar" "box" "slimbox"; "bottombox" is added to osc_lazy
    ...
    timetotal = true,                   -- display total time instead of remaining time? -- default is false
    timems = false,                     -- display timecodes with milliseconds?
    visibility = "auto",                -- only used at init to set visibility_mode(...)
    boxmaxchars = 150,                  -- title crop threshold for box layout           -- default is 80
    ...
    sub_title2 = "Contrast[${contrast}] Brightness[${brightness}] Gamma[${gamma}] Saturation[${saturation}] Hue[${hue}]", -- Temporary right subtitle of the bottombox layout
    seekbar_scrollseek = "fast",        -- the scroll wheel jump mode of the progress bar is "fast" "second" "frame". Not controlled by global hr-seek
    showonpause = false,                -- permanent OSC during pause
    showonstart = false,                -- display OSC at the beginning of playback or when playing the next file
    showonseek = false,                 -- display OSC when jumping
    font = "sans",                      -- global font display for OSC
    font_mono = "sans",
    font_bold = 500,
}
...
function get_tracklist(type)
    local msg = "Available" .. nicetypes[type] .. "Track:"
...
function render_elements(master_ass)
    ...
    -- temporary display of subheadings on the right side of the bottombox
...
function osc_init()
    ...
    --cy_audio --global audio track button enhancements
    ...
    --tog_fs --toggle fullscreen/windowed
    ...
    ne.eventresponder["mouse_move"] = --keyframe seeking when mouse is dragged
        function (element)
            ...
            -- identical seeks
            thumbfast.pause = false --pause rendering thumbnails
            mp.commandv("script-message-to", "thumbfast", "clear")
            local seekto = get_slider_value(element)
            if (element.state.lastseek == nil) or
                (not (element.state.lastseek == seekto)) then
                    local flags = "absolute-percent+keyframes" -- prevent the effects of --hr-seek
                    if not user_opts.seekbarkeyframes then
                        flags = "absolute-percent+exact"       -- prevent the effects of --hr-seek
                    end
    ...
    -- tc_right (total/remaining time) -- total/time remaining
...
function tick()
    ...
    if (state.idle) then
        ...
        if user_opts.idlescreen then
            ...
            ass:an(8)
            ass:append("Drag and drop in files/directories/URLs to play")
        end
