...
function stripfilename(pathfile, media_title)
    ...
    local protocol = is_protocol(pathfile)
...
function get_name_from_index(i, notitle)
    ...
    if title and not notitle and should_use_title then
        ...
        return url_decode(stripfilename(title, true)):gsub("\\", '\\\239\187\191'):gsub("{", "\\{"):gsub("^ ", "\\h")
    end
    ...
    return url_decode(stripfilename(name)):gsub("\\", '\\\239\187\191'):gsub("{", "\\{"):gsub("^ ", "\\h")
end
...
function add_keybinds()
    ...
    bind_keys_forced(settings.key_sortplaylist, "sortplaylist", sortplaylist)
