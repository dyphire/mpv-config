...
if screenshot_dir ~= '' then
    ...
    if not meta or not meta.is_dir then
        local windows_args = { 'powershell', '-NoProfile', '-Command', 'mkdir', string.format("\"%s\"", screenshot_dir) }
...
local function perform_capture(idx, crop_arg_local, line)
	...
	if result and result.status == 0 then
		...
		mp.osd_message("Cropping failed: " .. (result and result.error or "unknown"))
	end
...
function DisplayState:recalculate_bounds(forced)
	...
	if not (forced or self.current_state == nil) then
		local changed = false
		for k in pairs(new_state) do if new_state[k] ~= self.current_state[k] then changed = true break end end
	...
	if new_state.video_w and new_state.video_h then
		...
		if new_state.keepaspect then
			...
			self.bounds = { left = video_left, right = video_right, top = video_top, bottom = video_bottom, width = video_right - video_left, height = video_bottom - video_top }
		else
			self.bounds = { left = 0, top = 0, right = self.screen.width, bottom = self.screen.height, width = self.screen.width, height = self.screen.height }
		end
		self.scale.x = (new_state.video_w_raw or new_state.video_w) / math.max(1, self.bounds.width)
		self.scale.y = (new_state.video_h_raw or new_state.video_h) / math.max(1, self.bounds.height)
...
local function asscrop_update()
	...
	if asscrop_start_pos then
		...
		local format_dim = string.format("{\\bord0\\1a&H%02X&\\1c&H%02X%02X%02X&}", overlay_transparency, overlay_lightness, overlay_lightness, overlay_lightness)
	    ...
	    local box_format = string.format("{\\1a&HFF&\\1c&H000000&\\3c&H%02X%02X%02X&\\bord1}", line_color, line_color, line_color)
		ass:new_event()
		ass:pos(0,0)
...
local function asscrop_finish()
	...
	do
		...
		if not tx0 or not tx1 then mp.osd_message("Unable to determine video coordinates") return end
		vx0, vy0 = tx0, ty0
		vw = math.abs(tx1 - tx0)
		vh = math.abs(ty1 - ty0)
	end

	if vw <= 0 or vh <= 0 then mp.osd_message("Bad crop size") return end

	-- If a callback is registered (used by take_screenshot first-shot flow), call it with crop and skip mpv crop
...
local function take_screenshot()
	...
	local v_w = mp.get_property_number("video-params/w") or mp.get_property_number("width", 0)
	local v_h = mp.get_property_number("video-params/h") or mp.get_property_number("height", 0)
...
local function stitch_images()
	...
	if result.status == 0 then
		...
		mp.osd_message("Stitching failed: " .. (result and result.error or "unknown"))
		return
	end
