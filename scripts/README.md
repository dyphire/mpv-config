...
1. 部分脚本为**个人修改版本**，主要改进功能实现或键位绑定方式。如：autosubsync（组）; contextmenu_gui（组）; autoload.lua; chapter_list.lua; chapterskip.lua; copy_subortime.lua; dynamic-crop.lua; editions-notification.lua; file-browser.lua; fuzzydir.lua; history-bookmark.lua; persist_properties.lua; skiptosilence.lua; trackselect.lua; thumbnailer*.lua; ytdl_hook.lua
