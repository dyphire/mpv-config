...
function TopBar:render()
	...
	if options.top_bar_title and (state.title or state.has_playlist) then
		...
		-- Alt title
		local function replace(str) str = string.gsub(str, "[%(%)%.%+%-%*%?%[%]%^%$%%]", "%%%1") return str end
		if state.alt_title and options.top_bar_alt_title_place == 'below' and state.is_idle == false
		and string.match(state.title, replace(state.alt_title)) == nil then
